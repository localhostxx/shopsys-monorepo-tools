...
# Final monorepo will contain all branches from the first remote and platform branches of all remotes will be merged
...
for PARAM in $@; do
    ...
    # Rewrite all branches from the first remote, only platform branches from others
    if [ "$PARAM" == "$1" ]; then
        ...
        echo "Building branch 'platform' of the remote '$REMOTE'"
        git checkout --detach $REMOTE/platform
...
for REF in $MERGE_REFS; do
    # Add all files from all platform branches into index
