...
# Split monorepo and push all platform branches and all tags into specified remotes
...
for PARAM in $@; do
    ...
    # Rewrite git history of platform branch
    echo "Splitting repository for the remote '$REMOTE'"
    git checkout platform
    $MONOREPO_SCRIPT_DIR/rewrite_history_from.sh $SUBDIRECTORY platform $(git tag)
    if [ $? -eq 0 ]; then
        echo "Pushing branch 'platform' and all tags into '$REMOTE'"
        git push --tags $REMOTE platform
    else
